...
function mostrarFormularioObra(obra = null) {
  obrasTab.innerHTML = `
    <div class="card shadow-sm p-4 mb-4 bg-light">
      <h4 class="mb-3 text-${obra ? "warning" : "primary"}">${
    obra ? "Editar Obra Social" : "Nueva Obra Social"
  }</h5>
      <div class="row g-3">
        <div class="col-md-6">
          <label class="form-label" for='nombreObra'>Nombre:</label>
          <input type="text" id="nombreObra" class="form-control" value="${
            obra ? obra.nombre : ""
          }">
        </div>
        <div class="col-md-6">
          <label class="form-label" for='descripcionObra'>Descripción:</label>
          <textarea id="descripcionObra" class="form-control">${
            obra ? obra.descripcion : ""
          }</textarea>
        </div>
        <div class="col-md-6">
          <label class="form-label" for='porcentajeObra'>Porcentaje de descuento (%):</label>
          <input type="number" id="porcentajeObra" class="form-control" min="0" max="100" value="${
            obra ? obra.porcentaje : 0
          }">
          </div>
      </div>
      <div class="mt-4">
        <button class="btn btn-${
          obra ? "warning" : "success"
        } me-2" id="guardarObraBtn">${
    obra ? "Guardar Cambios" : "Agregar Obra"
  }</button>
...
document.addEventListener("click", (e) => {
  if (e.target && e.target.id === "guardarObraBtn") {
    ...
    const porcentaje = parseFloat(
      document.getElementById("porcentajeObra").value
    );
    ...
    if (e.target.dataset.id) {
      ...
      const nuevoId =
        obrasSocialesStorage.length > 0
          ? Math.max(...obrasSocialesStorage.map((o) => parseInt(o.id) || 0)) +
            1
          : 1;

      obrasSocialesStorage.push({
        id: nuevoId,
        nombre,
        descripcion,
        porcentaje,
  ...
  if (e.target && e.target.classList.contains("eliminarObraBtn")) {
    ...
    if (confirm("¿Desea eliminar esta obra social?")) {
      ...
      localStorage.setItem(
        "obrasSociales",
        JSON.stringify(obrasSocialesStorage)
      );
      mostrarObras();
    }
  }
